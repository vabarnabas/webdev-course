...
const App = (props) => {
  ...
  return (
    <div className='min-h-screen w-full flex justify-center items-center bg-editorDark font-menlo text-textWhite px-6'>
      ...
      <ReactMarkdown className='prose prose-sm prose-code:max-w-full overflow-x-auto prose-headings:text-textBlue prose-p:text-textWhite prose-em:text-textRed prose-strong:text-textPurple prose-pre:bg-editorLight' escapeHTML={true} children={markdown} />
      </div>
    </div>
  )
}
